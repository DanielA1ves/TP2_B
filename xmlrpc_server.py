...
class XMLRPCHandler:
    ...
    def execute_xpath(self, xpath_query):
        """Executa uma query XPath no XML com tratamento de erros específico"""
        try:
            # Forçar conversão a numérico corretamente
            # e evitar falhas ao comparar strings com números
            result = self.root.xpath(xpath_query, smart_strings=False)
            if isinstance(result, list):
                return [
                    etree.tostring(item, encoding='unicode') if isinstance(item, etree._Element)
                    else str(item)
                    for item in result
                ]
            return str(result)
            ...
            return f"Erro ao executar XPath (eval): {str(e)}"
    ...
    def get_all_records(self):
        ...
        records = self.root.xpath('//property')
    ...
    def count_records(self):
        ...
        return len(self.root.xpath('//property'))
...
if __name__ == '__main__':
    import sys
    xml_file = sys.argv[1] if len(sys.argv) > 1 else 'house_purchase.xml'
    start_server(xml_path=xml_file)